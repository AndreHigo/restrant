...
const operationSections: readonly NavigationSection[] = [
  {
    ...
    items: [
      ...
      { href: "/operacao/balcao", label: "Balcao", icon: UtensilsIcon, permission: "sales.view", operationFlag: "enableCounter" },
  ...
  return (
    <div className="min-h-screen bg-[#f6f5ef]">
      <div className="grid min-h-screen lg:grid-cols-[292px_minmax(0,1fr)]">
        <aside className="hidden border-r border-slate-200 bg-slate-950 px-5 py-6 text-white lg:sticky lg:top-0 lg:block lg:h-screen lg:overflow-y-auto">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Restaurant Brasil</p>
            <h1 className="mt-2 text-[24px] font-semibold leading-tight">Gestao completa</h1>
            <p className="mt-2 text-sm leading-5 text-slate-300">Operacao, caixa e gestao em uma base.</p>
          </div>
          ...
          <nav className="mt-7 space-y-3">
            {sections.map((section) => (
              ...
              >
                <summary className="flex min-h-9 cursor-pointer list-none items-center justify-between gap-3 rounded-lg px-3 text-left text-slate-200 transition hover:bg-white/[0.06]">
                  <span className="text-[13px] font-semibold uppercase tracking-[0.12em]">
                    {section.label}
                  </span>
                  <span className="flex h-6 w-6 items-center justify-center rounded-md bg-white/[0.06] text-sm font-semibold text-slate-300 transition group-open:bg-white/10 group-open:text-white">
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="group/link flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium leading-none text-slate-100 transition hover:bg-white/10 hover:text-white"
                      >
          ...
          <div className="mt-7 rounded-lg border border-white/10 bg-white/5 p-3">
            ...
            <p className="mt-2 text-xs leading-5 text-slate-400">Acesso filtrado por permissao.</p>
          </div>
        </aside>

        <main className="min-w-0 p-3 sm:p-4 lg:p-6">
          <section className="mb-3 rounded-xl bg-slate-950 p-4 text-white shadow-panel lg:hidden">
          ...
          <div className="min-w-0 rounded-xl bg-white shadow-panel">
            <header className="border-b border-slate-100 px-4 py-4 sm:px-5 lg:px-6">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-700">{area}</p>
