...
process.env.NFCE_CANCEL_MOCK = "authorized";
...
const certificatePassword = "qa-cancelamento-a1";
...
async function createTemporaryPfx(fileName: string) {
  ...
  const attrs = [
    { name: "commonName", value: "Restaurant Brasil QA Cancelamento" },
...
async function main() {
  ...
  const certificatePath = await createTemporaryPfx(`qa-cancelamento-${suffix}.pfx`);

  try {
    await db.companySetting.update({
      where: { id: company.id },
      data: {
        fiscalCertificateName: `qa-cancelamento-${suffix}.pfx`,
    ...
    const product = await db.product.create({
      data: {
        ...
        fiscalCfop: "5102",
        fiscalNcm: "21069090",
        name: `QA Cancelamento NFCe ${suffix}`,
        price: 21.5,
        sku: `96${suffix}`,
        trackStock: false,
        type: "READY",
      ...
      data: {
        ...
        items: {
          create: {
            ...
            totalPrice: total,
            unitPrice: total,
            notes: "QA cancelamento NFC-e"
          }
        },
        notes: "QA cancelamento NFC-e",
        number: `NFCE-CAN-${suffix}`,
        openedBy: user.id,
    ...
    const canceled = await postJson("/api/admin/fiscal/nfce/cancel", cookieHeader, {
      fiscalDocumentId: prepared.id,
      justification: "Cancelamento de teste automatizado em homologacao.",
      mockAuthorized: true
    });
    ...
    const results = [
      ...
      {
        detail: canceled.transmissionStatus,
        label: "cancelamento",
        ok: canceled.transmissionStatus === "CANCELED"
      }
    ];
    ...
    if (failed.length > 0) {
      throw new Error(`Cancelamento NFC-e falhou: ${failed.map((item) => item.label).join(", ")}`);
    }

    console.log("Cancelamento NFC-e homologacao aprovado em modo mock.");
  } finally {
