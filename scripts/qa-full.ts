...
async function main() {
  console.log("1/18 Build de producao");
  runNpmScript("build");

  console.log(`2/18 Subindo servidor temporario em ${baseUrl}`);
  ...
  try {
    ...
    console.log("3/18 Smoke test das rotas criticas");
    runNpmScript("test:smoke", testEnv);

    console.log("4/18 Smoke test do fluxo operacional");
    runNpmScript("test:flow", testEnv);

    console.log("5/18 Smoke test de indices operacionais");
    runNpmScript("test:operational-indexes", testEnv);

    console.log("6/18 Smoke test de carga leve operacional");
    runNpmScript("test:operational-load", testEnv);

    console.log("7/18 Smoke test de carga leve de escrita");
    runNpmScript("test:operational-write-load", testEnv);

    console.log("8/18 Smoke test de divisao parcial de conta");
    runNpmScript("test:partial-payment", testEnv);

    console.log("9/18 Smoke test de atendimento rapido de balcao");
    runNpmScript("test:counter", testEnv);

    console.log("10/18 Smoke test de bloqueios por modos operacionais");
    runNpmScript("test:operation-modes", testEnv);

    console.log("11/18 Smoke test de bloqueio de peso manual por permissao");
    runNpmScript("test:manual-weight", testEnv);

    console.log("12/18 Smoke test de captura automatica de peso estavel");
    runNpmScript("test:scale-stable", testEnv);

    console.log("13/18 Smoke test de bloqueio de venda sem estoque");
    runNpmScript("test:stock-block", testEnv);

    console.log("14/18 Smoke test de auditoria de estoque");
    runNpmScript("test:stock-audit", testEnv);

    console.log("15/18 Smoke test de cancelamento auditado de compra");
    runNpmScript("test:purchase-cancel", testEnv);

    console.log("16/18 Smoke test de aprovacao de cancelamento");
    runNpmScript("test:cancellation-approval", testEnv);

    console.log("17/18 Smoke test de permissoes e bloqueios RBAC");
    runNpmScript("test:rbac", testEnv);

    console.log("18/18 Simulacao completa de restaurante");
