...
async function main() {
  console.log("1/15 Build de producao");
  runNpmScript("build");

  console.log(`2/15 Subindo servidor temporario em ${baseUrl}`);
  ...
  try {
    ...
    console.log("3/15 Smoke test das rotas criticas");
    runNpmScript("test:smoke", testEnv);

    console.log("4/15 Smoke test do fluxo operacional");
    runNpmScript("test:flow", testEnv);

    console.log("5/15 Smoke test de divisao parcial de conta");
    runNpmScript("test:partial-payment", testEnv);

    console.log("6/15 Smoke test de atendimento rapido de balcao");
    runNpmScript("test:counter", testEnv);

    console.log("7/15 Smoke test de bloqueios por modos operacionais");
    runNpmScript("test:operation-modes", testEnv);

    console.log("8/15 Smoke test de bloqueio de peso manual por permissao");
    runNpmScript("test:manual-weight", testEnv);

    console.log("9/15 Smoke test de captura automatica de peso estavel");
    runNpmScript("test:scale-stable", testEnv);

    console.log("10/15 Smoke test de bloqueio de venda sem estoque");
    runNpmScript("test:stock-block", testEnv);

    console.log("11/15 Smoke test de auditoria de estoque");
    runNpmScript("test:stock-audit", testEnv);

    console.log("12/15 Smoke test de cancelamento auditado de compra");
    runNpmScript("test:purchase-cancel", testEnv);

    console.log("13/15 Smoke test de aprovacao de cancelamento");
    runNpmScript("test:cancellation-approval", testEnv);

    console.log("14/15 Smoke test de permissoes e bloqueios RBAC");
    runNpmScript("test:rbac", testEnv);

    console.log("15/15 Simulacao completa de restaurante");
